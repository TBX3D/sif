...
// hash, or one out of 32-bit range) at load rather than at match time. it also
// rejects a typo'd encoding and a malformed range matcher for the same reason,
// transport-agnostically (http, dns, and tcp all route through here).
func validateMatchers(matchers []Matcher) error {
	for i := range matchers {
		...
		if e := strings.ToLower(matchers[i].Encoding); e != "" && e != "base64" && e != "hex" {
			return fmt.Errorf("matcher encoding %q not supported (use base64 or hex)", matchers[i].Encoding)
		}
