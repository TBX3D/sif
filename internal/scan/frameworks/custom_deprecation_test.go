...
// TestLegacyCustomSignatureDeprecationLogged extends the C5 backcompat proof
// (TestLegacyCustomSignatureStillLoads / custom_backcompat_test.go): the
// legacy signatures/ dir must keep loading AND now also emit one deprecation
// notice steering users at the unified fingerprint-module surface (O2). it
// must not fail, stop loading, or migrate anything.
