...
func (app *App) Run(ctx context.Context) error {
	...
	for _, url := range app.targets {
		...
		if app.settings.ApiMode && ts.apiOutput != "" {
			fmt.Println(ts.apiOutput)
		}
	}
...
// targetScan is one target's isolated scan output: every slice a single scan
// touches, collected locally so a caller can merge many targets without a
// shared accumulator.
...
// its isolated results. it never touches run-wide state, so callers can run
// it sequentially or from a worker pool without additional locking.
func (app *App) scanTarget(ctx context.Context, url, storeDir string, wantReport bool) (targetScan, error) {
	...
	if app.settings.LogDir != "" {
		if err := logger.CreateFile(&ts.logFiles, url, app.settings.LogDir); err != nil {
			return ts, err
		}
	}
	...
	if !app.settings.NoScan {
		...
		ts.scansRun = append(ts.scansRun, "Basic Scan")
	}
	...
	if app.settings.Framework {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "Framework Detection")
		}
	}
	...
	if app.settings.Dirlist != "none" {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Directory Listing")
		}
	}
	...
	if app.settings.Dnslist != "none" {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "DNS Scan")
		}
		...
		if app.settings.SubdomainTakeover {
			...
			if err != nil {
				...
				ts.scansRun = append(ts.scansRun, "Subdomain Takeover")
			}
		}
	...
	if app.settings.Dorking {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "Dork")
		}
	}
	...
	if app.settings.Ports != "none" {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "Port Scan")
		}
	}

	if app.settings.Whois {
		scan.Whois(url, app.settings.LogDir)
		ts.scansRun = append(ts.scansRun, "Whois")
	}
	...
	if app.settings.Git {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "Git")
		}
	}
	...
	if app.settings.Nuclei {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "Nuclei")
		}
	}
	...
	if app.settings.JavaScript {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "JS")
		}
	}
	...
	if app.settings.CMS {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "CMS")
		} else if result != nil {
			moduleResults = append(moduleResults, NewModuleResult(result))
		}
	}
	...
	if app.settings.Headers {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "HTTP Headers")
		}
	}
	...
	if app.settings.SecurityHeaders {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Security Headers")
		}
	}
	...
	if app.settings.CloudStorage {
		...
		if err != nil {
			...
			ts.scansRun = append(ts.scansRun, "Cloud Storage")
		}
	}
	...
	if app.settings.Shodan {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Shodan")
		}
	}
	...
	if app.settings.SQL {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "SQL Recon")
		}
	}
	...
	if app.settings.LFI {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "LFI Recon")
		}
	}
	...
	if app.settings.JWT {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "JWT")
		}
	}
	...
	if app.settings.OpenAPI {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "OpenAPI")
		}
	}
	...
	if app.settings.Favicon {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Favicon")
		}
	}
	...
	if app.settings.CORS {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "CORS")
		}
	}
	...
	if app.settings.Redirect {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Open Redirect")
		}
	}
	...
	if app.settings.XSS {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Reflected XSS")
		}
	}
	...
	if app.settings.Crawl {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Crawl")
		}
	}
	...
	if app.settings.Passive {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Passive")
		}
	}
	...
	if app.settings.Probe {
		...
		if err != nil {
			...
			moduleResults = append(moduleResults, NewModuleResult(result))
			ts.scansRun = append(ts.scansRun, "Probe")
		}
	}
	...
	if app.settings.ApiMode {
		...
		ts.apiOutput = string(marshalled)
	}

	targetFindings := collectFindings(url, moduleResults)
	ts.findings = append(ts.findings, targetFindings...)
	...
	if wantReport {
		ts.reportResults = append(ts.reportResults, collectReportResults(url, moduleResults)...)
	}
